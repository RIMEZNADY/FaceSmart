...
class manag:
    ...
    def open_deptform(app):
        # This function will destroy the current CTk window and open a new script.
            app.destroy()
            os.system('python C:\\Users\\Dell\\OneDrive\\Desktop\\finalpfa\\finalpfa\\pfa\\dept_form.py')
    ...
    def fetch_data(self):
        ...
        if self.conn is not None and self.conn.is_connected():
            try:
                ...
                cursor.execute("SELECT dept_id, dept_name FROM pfa_db.departments")
    ...
    #     initial_headers = [["Departement ID", "Departement Type", "Action"]]
    #     full_data = initial_headers + [[str(row[0]), row[1], DeleteButton(master=self.app, row_id=row[0], command=lambda r=row[0]: self.delete_row(r))] for row in data]
    ...
    def populate_table(self, data):
        ...
        # Add headers
        headers = [["Departement ID", "Departement Type", "Action"]]
    ...
    def delete_row(self, row_id):
        ...
        if self.conn is not None and self.conn.is_connected():
            try:
                cursor = self.conn.cursor()
                cursor.execute("DELETE FROM pfa_db.departments WHERE dept_id = %s", (row_id,))
                self.conn.commit()
                cursor.close()
    ...
    def on_table_select(self, event):
        ...
        if selected_row and selected_row['values'] is not None:
            try:
                row_id = selected_row['values'][0]
                print("Selected Row ID:", row_id)
            except IndexError:
    ...
    def setup_ui(self):
        ...
        CTkLabel(master=title_frame, text="Departements", font=("Arial Black", 25), text_color="#87CEEB").pack(anchor="nw", side="left")
        ...
        CTkButton(master=search_container, text="ADD", font=("Arial Black", 15), text_color="#FCEDDA", fg_color="#87CEEB",
                hover_color="#0463CA" , command=lambda: manag.open_deptform(app)).pack(anchor="ne", side="left", padx=(13, 0), pady=15)
